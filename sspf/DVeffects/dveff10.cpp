...
class DVEffect10 : public VEffect
{
public:
    DVEffect10(char const *tag, int32 e);
    virtual         ~DVEffect10();
    virtual void    Init();
...
Vertex  DVEffect10::s_localVertices[2][4 * 8];
uint16  DVEffect10::s_localIndices[6 * 8];

#ifdef NDEBUG
dec(DVEffect10, dveffect10_0,  0);
dec(DVEffect10, dveffect10_1,  1);
dec(DVEffect10, dveffect10_2, -1);
#endif

DVEffect10::DVEffect10(char const *tag, int32 e)
{
    m_edge = e;
    m_count = 2;
    m_usages = IS_DOUBLE;
...
DVEffect10::~DVEffect10()
{
}

void    DVEffect10::Init()
{
...
void    DVEffect10::GenerateTrans(AATrans *transforms, uint32 w, uint32 h)
{
    m_transforms = transforms;
    memset(&transforms[0], 0, 2 * sizeof(AATrans));
    transforms[0].postTrans.y = rand32() & 1 ? 2.0f / 256.0f : -2.0f / 256.0f;
    transforms[1].postTrans.y = -transforms[0].postTrans.y;
}

void    DVEffect10::GenerateFrames(float frameNum, float *frames, float *frames_p2) const
{
...
void    DVEffect10::GetTransData(int32 pass, float frameNum, uint32 *baseTrans, uint32 *startIndex, uint32 *numTri, uint32 *numTri_p2, uint8 *alpha, D3D11_RECT const **scissor) const
{
    if (numTri != NULL)
    {
        *numTri = 16;
