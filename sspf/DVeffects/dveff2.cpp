// Vertical split two piece slide of old image out and new one in.
...
class DVEffect2 : public VEffect
{
public:
    DVEffect2(char const *tag, int32 m, int32 t);
    virtual             ~DVEffect2();
    virtual void        Init();
...
D3D11_RECT  DVEffect2::s_rects[2];
uint16      DVEffect2::s_localIndices[6 * 2];

#ifdef NDEBUG
dec(DVEffect2, dveffect2_0, 0, 0);
dec(DVEffect2, dveffect2_1, 1, 0);
dec(DVEffect2, dveffect2_2, 2, 0);
dec(DVEffect2, dveffect2_3, 3, 0);
dec(DVEffect2, dveffect2_4, 0, 1);
dec(DVEffect2, dveffect2_5, 1, 1);
dec(DVEffect2, dveffect2_6, 2, 1);
dec(DVEffect2, dveffect2_7, 3, 1);
#endif

DVEffect2::DVEffect2(char const *tag, int32 m, int32 t)
{
    m_mode = m;
    m_type = t;
    m_passes = 2;
...
void     DVEffect2::GenerateTrans(AATrans *transforms, uint32 w, uint32 h)
{
    float xFactors[2];
    GenerateRectangles(m_localVertices[0], 2, 1, s_rects, w, h, xFactors, NULL);
    ...
    default:
    case 0:
        transforms[1].postTrans.x = xFactors[0] / 256.0f;
        transforms[4].postTrans.x = xFactors[1] / 256.0f;
        break;
    ...
    case 2:
        ...
        transforms[4].postTrans.y = -2.0f / 256.0f;
        break;
...
void    DVEffect2::GetTransData(int32 pass, float frameNum, uint32 *baseTrans, uint32 *startIndex, uint32 *numTri, uint32 *numTri_p2, uint8 *alpha, D3D11_RECT const **scissor) const
{
    if (baseTrans != NULL)
    {
        *baseTrans = pass * 3;
