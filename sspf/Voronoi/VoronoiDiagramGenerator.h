...
public:
	...
	bool getNext(float& x1, float& y1, float& x2, float& y2, int &i1, int &i2)
	{
		if(iteratorEdges == 0)
			return false;
...
private:
	...
	void pushGraphEdge(float x1, float y1, float x2, float y2, int i1, int i2);
...
extern VoronoiDiagramGenerator  generator;
